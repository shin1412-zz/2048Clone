...
@objc protocol Game2048Delegate {
    // Game flow and rules
    @objc optional func game2048ScoreChanged(_ game: Game2048, score: Int)
...
class Game2048: NSObject {
    ...
    func swipe(_ axis: SwipeAxis) {
        switch axis {
        case .x(let d):
            process(d, yCompareDirection: .none)
        case .y(let d):
            process(.none, yCompareDirection: d)
        }
    }
    ...
    fileprivate func process(_ xCompareDirection: Direction, yCompareDirection: Direction) {
        ...
        if model.indecies(of: 0).count <= 0 && !model.moceIsPossible {
